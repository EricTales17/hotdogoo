...
class DogaoPro:
    def __init__(self, db_name='hotdog_v2.db'):
        self.db_name = db_name
        self.usuario_logado = None # Guarda (id, nome, papel)
        self.configurar_banco()
    ...
    def configurar_banco(self):
        with self.conectar() as conn:
            ...
            ''')
            # Tabela de Vendas (agora com usuario_id)
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS vendas (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    usuario_id INTEGER,
            ...
            if not cursor.fetchone():
                cursor.execute('INSERT INTO usuarios (login, senha, papel) VALUES (?, ?, ?)', 
                               ('admin', 'admin123', 'admin'))
                # Itens iniciais
                itens = [('Dog Simples', 12.0, 50), ('Dog Completo', 18.0, 30), ('Refri Lata', 6.0, 100)]
                cursor.executemany('INSERT INTO produtos (nome, preco, estoque) VALUES (?, ?, ?)', itens)
            
            conn.commit()

    # --- SISTEMA DE AUTENTICAÇÃO ---
    
    def realizar_login(self, login, senha):
        with self.conectar() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT id, login, papel FROM usuarios WHERE login = ? AND senha = ?', (login, senha))
